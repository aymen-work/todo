            ...
            <form @submit.prevent="title==='Update'?updateTask() : addTask()" class="max-w-sm mx-auto">
                ...
                <div class="mb-5">
                    ...
                    <textarea id="details" v-model="form.details"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg  block w-full p-2.5 h-32 resize-none focus:outline-blue-600 "
                        required></textarea>
                    <label for="details" class="block mb-2 font-medium text-gray-900 text-xl">End date</label>

                    <input type="datetime-local" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5 focus:outline-blue-600" v-model="form.end_date">
                </div>
